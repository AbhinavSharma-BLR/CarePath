  ...
  try {
    ...
    if (!authHeader) {
      return NextResponse.json(
        { error: 'Unauthorized', message: 'Authentication required. Please log in as a Doctor.' },
        { status: 401 }
      )
    }

    const res = await fetch(`${API_BASE_URL}/queue/call`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      { error: 'Internal Server Error', message: 'Failed to call patient' },
      { status: 500 }
    )
  }
}
