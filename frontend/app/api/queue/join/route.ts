  ...
  try {
    ...
    if (!authHeader) {
      return NextResponse.json(
        { error: 'Unauthorized', message: 'Authentication required to join virtual queue. Please log in as a Patient.' },
        { status: 401 }
      )
    }
    ...
    return NextResponse.json(
      { error: 'Internal Server Error', message: 'Failed to join queue' },
      { status: 500 }
    )
  }
}
